...
def gerenciar_evento(request):
    if request.method == "GET":
        nomeFiltro = request.GET.get('nome')
        eventos = Evento.objects.filter(criador=request.user)

        #para fazer: outros filtros
        if nomeFiltro:
            eventos = eventos.filter(nome__contains=nomeFiltro)
...
def inscrever_evento(request, id):
    ...
    if request.method == "GET":
        ...
        #validar se o usuário ja esta participando
...
def gerar_csv(request, id):
    ...
    with open(path, 'w') as arq:
        ...
        for participantes in participantes:
            x = (participantes.username, participantes.email)
            writer.writerow(x)
...
def certificado_evento(request, id):
    evento = get_list_or_404(Evento, id=id) 
    if not evento.criador == request.user:
        raise Http404('Este evento não é seu')
...
def gerar_certificado(request, id):
    ...
    path_template = os.path.join(settings.BASE_DIR, 'template/static/evento/img/template_certificado.png')
    path_fonte = os.path.join(settings.BASE_DIR, 'template/static/fontes/arimo.ttf')

    for participante in evento.participantes.all():
        #para fazer: validar se o certicicado ja foi gerado
        ...
        draw.text((761, 782), f"{evento.nome}", font=font_nome, fill=(0,0,0))
        draw.text((816, 849), f"{evento.carga_horaria} horas", font=font_nome, fill=(0,0,0))

        output = BytesIO()
        img.save(output, format="PNG", quality=100)
        output.seek(0)

        img_final = InMemoryUploadedFile(output, 'ImageFiel', f'{token_urlsafe(8).png}', 'image/jpeg', sys.getsizeof(output), None)
    ...
    return redirect(reverse('certificado_evento', kwargs={'id': evento.id}))
